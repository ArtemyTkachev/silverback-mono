...
test('a process can be killed', async () => {
  ...
  expect(killedAfterMs).lessThan(1000);
});
...
test('a stuck process can be killed', async () => {
  ...
  const killedAfterMs = Date.now() - startedAtMs;
  expect(killedAfterMs).greaterThan(1000);
});
...
test('exit code of a failed command is logged', async () => {
  ...
  expect(output).toStrictEqual([
    ...
    '/bin/bash: I_DO_NOT_EXIST: command not found\n',
...
test('outputTimeout is exceeded', async () => {
  ...
  await process.result;
  expect(output).toStrictEqual([
    'ℹ️ Starting command: "sleep 0.2"\n',
    '⚠️ Killing command due to the output timeout (100ms): "sleep 0.2"\n',
    'ℹ️ Killing command: "sleep 0.2"\n',
    '❌ Command exited with null: "sleep 0.2"\n',
  ]);
});
